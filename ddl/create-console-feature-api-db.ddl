...
create table FEATURE (FEATURE_ID varchar(255) not null, DEPLOYED_DATE varchar(255), DESCRIPTION varchar(255), PROMOTED boolean not null, STATUS varchar(255), TITLE varchar(255), UPDATED_DATE varchar(255) not null, primary key (FEATURE_ID));
...
create table REO_COMMIT (MATCHED_TYPE varchar(31) not null, REPO_COMMIT_ID int8 not null, SHA varchar(255) not null, REPO_ID varchar(255) not null, FEATURE_ID varchar(255) not null, primary key (REPO_COMMIT_ID));
create table REPO (REPO_ID varchar(255) not null, MICROSERVICE varchar(255) not null, primary key (REPO_ID));
create table REPO_FEATURE (REPO_ID varchar(255) not null, FEATURE_ID varchar(255) not null, primary key (REPO_ID, FEATURE_ID));
...
alter table REPO_FEATURE add constraint REPO_FEATURE_FEATURE_ID_FK foreign key (FEATURE_ID) references FEATURE;
alter table REPO_FEATURE add constraint REPO_FEATURE_REPO_ID_FK foreign key (REPO_ID) references REPO;
