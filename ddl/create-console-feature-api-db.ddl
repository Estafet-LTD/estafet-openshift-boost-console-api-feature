create sequence COMMIT_DATE_ID_SEQ start 1 increment 1;
...
create table REPO_COMMIT (MATCHED_TYPE varchar(31) not null, REPO_COMMIT_ID int8 not null, MESSAGE varchar(255) not null, SHA varchar(255) not null, VERSION varchar(255), REPO_ID varchar(255) not null, FEATURE_ID varchar(255), primary key (REPO_COMMIT_ID));
alter table COMMIT_DATE add constraint COMMIT_DATE_KEY unique (REPO_ID, SHA);
...
alter table COMMIT_DATE add constraint COMMIT_DATE_TO_REPO_FK foreign key (REPO_ID) references REPO;
